...
int process_conversion_job(const ConversionJob* job) {
    ...
    // Build command with proper environment setup for fallback logging
    int ret = snprintf(command, sizeof(command), 
                      "export PROJECT_ROOT='%s' && "
                      "export LOG_FILE='%s/logs/history.log' && "
                      "export LOG_DIR='%s/logs' && "
                      "export LOGGING_INITIALIZED=1 && "
                      "export EXEC_MODE='thread' && "
                      "source '%s/lib/logging.sh' && "
                      "%s/lib/conversion.sh \"%s\"", 
                      cwd, cwd, cwd, cwd, cwd, job->filepath);
